...
static void print_issue(TCHAR *argv[])
{
	...
	locale_printf(LOC_ID_CRASS_REVISION, _T(CRAGE_VERSION));
	locale_printf(LOC_ID_CRASS_DATE, _T(CRAGE_RELEASE_TIME));
	locale_printf(LOC_ID_CRASS_RELEASE, CRAGE_RELEASE);
	locale_printf(LOC_ID_CRASS_SYNTAX, 
		argv[0] ? argv[0] : _T("Crage.exe"), CRAGE_SYNTAX);
}
...
static void parse_cmd(int argc, TCHAR *argv[])
{
	...
	for (i = 0; i < argc; i++) {
...
#ifndef UNICODE
			acp2unicode(argv[i + 1], -1, package_path, SOT(package_path));
#else
		...
		if (!_tcscmp(argv[i], _T("-d"))) {
			...
			conver_backslash(argv[i + 1]);
#ifndef UNICODE
			acp2unicode(argv[i + 1], -1, package_directory, SOT(package_directory));
#else
		...
		if (!_tcscmp(argv[i], _T("-o"))) {
			...
			conver_backslash(argv[i + 1]);
#ifndef UNICODE
			acp2unicode(argv[i + 1], -1, output_directory, SOT(output_directory));
#else
