											...
											struct package_directory *pkg_dir)
{
	...
	while (p_dir < directory + directory_length) {
		...
		if (name_offset != 0xf) {	// 只有部分名字
			...
			cur_ARC4a_offset = abs(int(cur_ARC4a_offset - bin_header.directory_offset));
											...
											struct package_directory *pkg_dir)
{
	...
	DWORD bsize;
	for (bsize = 0; bsize < lz3_header.length; ) {
		BYTE *tmp_b;
		DWORD tmp_bsize;
											...
											struct package_resource *pkg_res)
{
	...
	int ret;
	DWORD j;
	for (j = 1; j < my_ARC4_entry->number; ++j) {
		resource2_header_t res_header;
		if (pkg->pio->readvec(pkg, &res_header, sizeof(res_header),
	...
	if (!strncmp((char *)raw, "tZ", 2)) {
		...
		DWORD uncomprlen;
		for (uncomprlen = 0; uncomprlen < lz_header->total_uncomprlen; ) {
			DWORD act_comprlen;
