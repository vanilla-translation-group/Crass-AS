											   ...
											   struct package_directory *pkg_dir)
{
	...
	for (i = 0; i < index_entries; i++) {		
		...
		if (!index_buffer[i].name_crc) {
			...
			if (ext && *ext && (_ext = (char*)strstr(".HGO.ed8.edt.ogg.wav.ENV.bmp.exe.WAV.TAG.ED8.BAT.ITW.SUE.sal", ext))) {
				strncpy(ext, _ext, 4);
				ext[4] = 0;
											  ...
											  struct package_resource *pkg_res)
{
	...
	for (act_uncomprlen = 3; act_uncomprlen < uncomprlen; ) {
