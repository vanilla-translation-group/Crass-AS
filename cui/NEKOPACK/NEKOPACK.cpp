...
/* 接口数据结构: 表示cui插件的一般信息 */
struct acui_information NEKOPACK_cui_information = {
	NULL,					/* copyright */
	NULL,					/* system */
	_T(".dat"),				/* package */
	_T("1.1.2"),			/* revision */
	_T("痴漢公賊"),			/* author */
	_T("2009-6-28 0:06"),	/* date */
	NULL,					/* notion */
...
/* 封包匹配回调函数 */
static int NEKOPACK_old_dat_match(struct package *pkg)
{
...
/* 封包索引目录提取函数 */
static int NEKOPACK_old_dat_extract_directory(struct package *pkg,
...
/* 封包索引项解析函数 */
static int NEKOPACK_old_dat_parse_resource_info(struct package *pkg,
...
/* 封包资源提取函数 */
static int NEKOPACK_old_dat_extract_resource(struct package *pkg,
											 struct package_resource *pkg_res)
{
...
/* 资源保存函数 */
static int NEKOPACK_old_dat_save_resource(struct resource *res, 
										  struct package_resource *pkg_res)
{
...
/* 封包资源释放函数 */
static void NEKOPACK_old_dat_release_resource(struct package *pkg, 
											  struct package_resource *pkg_res)
{
...
/* 封包卸载函数 */
static void NEKOPACK_old_dat_release(struct package *pkg, 
									 struct package_directory *pkg_dir)
{
...
/* 封包处理回调函数集合 */
static cui_ext_operation NEKOPACK_old_dat_operation = {
	NEKOPACK_old_dat_match,					/* match */
	NEKOPACK_old_dat_extract_directory,		/* extract_directory */
	NEKOPACK_old_dat_parse_resource_info,	/* parse_resource_info */
	NEKOPACK_old_dat_extract_resource,		/* extract_resource */
	NEKOPACK_old_dat_save_resource,			/* save_resource */
	NEKOPACK_old_dat_release_resource,		/* release_resource */
	NEKOPACK_old_dat_release				/* release */
};
	
/********************* dat *********************/

/* 封包匹配回调函数 */
static int NEKOPACK_dat_match(struct package *pkg)
{
	s8 magic[8];
...
/* 封包索引目录提取函数 */
static int NEKOPACK_dat_extract_directory(struct package *pkg,
										  struct package_directory *pkg_dir)
{
	...
	BYTE *p;
	for (p = dec_index; p < dec_index + index_len; ) {
...
/* 封包资源提取函数 */
static int NEKOPACK_dat_extract_resource(struct package *pkg,
										 struct package_resource *pkg_res)
{
	u32 dec_exec_key;
	u64 enc_data_length;
...
/* 封包卸载函数 */
static void NEKOPACK_dat_release(struct package *pkg, 
								 struct package_directory *pkg_dir)
{
	if (pkg_dir->directory) {
		free(pkg_dir->directory);
...
/* 封包处理回调函数集合 */
static cui_ext_operation NEKOPACK_dat_operation = {
	NEKOPACK_dat_match,						/* match */
	NEKOPACK_dat_extract_directory,			/* extract_directory */
	NEKOPACK_old_dat_parse_resource_info,	/* parse_resource_info */
	NEKOPACK_dat_extract_resource,			/* extract_resource */
	NEKOPACK_old_dat_save_resource,			/* save_resource */
	NEKOPACK_old_dat_release_resource,		/* release_resource */
	NEKOPACK_dat_release					/* release */
};

/********************* new dat *********************/

/* 封包匹配回调函数 */
static int NEKOPACK_new_dat_match(struct package *pkg)
{
...
/* 封包索引目录提取函数 */
static int NEKOPACK_new_dat_extract_directory(struct package *pkg,
											  struct package_directory *pkg_dir)
{
	dat_new_header_t header;
	if (pkg->pio->readvec(pkg, &header, sizeof(header), 0, IO_SEEK_SET))
	...
	DWORD index_entries = 0;
	BYTE *p;
	for (p = index; p < index + header.index_length; ) {
...
/* 封包资源提取函数 */
static int NEKOPACK_new_dat_extract_resource(struct package *pkg,
										 struct package_resource *pkg_res)
{
	u32 parity;
...
/* 封包处理回调函数集合 */
static cui_ext_operation NEKOPACK_new_dat_operation = {
	NEKOPACK_new_dat_match,					/* match */
	NEKOPACK_new_dat_extract_directory,		/* extract_directory */
	NEKOPACK_old_dat_parse_resource_info,	/* parse_resource_info */
	NEKOPACK_new_dat_extract_resource,		/* extract_resource */
	NEKOPACK_old_dat_save_resource,			/* save_resource */
	NEKOPACK_old_dat_release_resource,		/* release_resource */
	NEKOPACK_dat_release					/* release */
};
...
/* 封包匹配回调函数 */
static int NEKOPACK_dat2_match(struct package *pkg)
{
	dat_new_header_t header;
...
/* 封包索引目录提取函数 */
static int NEKOPACK_dat2_extract_directory(struct package *pkg,
										   struct package_directory *pkg_dir)
{
...
/* 封包处理回调函数集合 */
static cui_ext_operation NEKOPACK_dat2_operation = {
	NEKOPACK_dat2_match,					/* match */
	NEKOPACK_dat2_extract_directory,		/* extract_directory */
	NEKOPACK_old_dat_parse_resource_info,	/* parse_resource_info */
	NEKOPACK_new_dat_extract_resource,		/* extract_resource */
	NEKOPACK_old_dat_save_resource,			/* save_resource */
	NEKOPACK_old_dat_release_resource,		/* release_resource */
...
int CALLBACK NEKOPACK_register_cui(struct cui_register_callback *callback)
{

	/* 注册cui插件支持的扩展名、资源放入扩展名、处理回调函数和封包属性 */
	if (callback->add_extension(callback->cui, _T(".dat"), NULL, 
		NULL, &NEKOPACK_new_dat_operation, CUI_EXT_FLAG_PKG | CUI_EXT_FLAG_DIR))
			return -1;

	if (callback->add_extension(callback->cui, _T(".dat"), NULL, 
		NULL, &NEKOPACK_old_dat_operation, CUI_EXT_FLAG_PKG | CUI_EXT_FLAG_DIR))
			return -1;

	if (callback->add_extension(callback->cui, _T(".dat"), NULL, 
		NULL, &NEKOPACK_dat_operation, CUI_EXT_FLAG_PKG | CUI_EXT_FLAG_DIR))
			return -1;

	if (callback->add_extension(callback->cui, _T(".dat"), NULL, 
		NULL, &NEKOPACK_dat2_operation, CUI_EXT_FLAG_PKG | CUI_EXT_FLAG_DIR))
			return -1;
