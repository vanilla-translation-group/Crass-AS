...
typedef struct {
	...
	u16 entries;
	u16 crc;
} DefFileForm_chunk_header_t;

typedef struct {
	u16 unknown;
	u16 string_length;		// ^ 0x73 for decoding (> 260 is invalid)
	u32 unknown1;
	u16 crc;
} DefFileForm_chunk_data_t;

typedef struct {
	s8 string[];
	u16 crc;
} DefFileForm_chunk_string_t;
	...
	for (i = 0; i < _4ag_header->index_entries; i++) {
		...
		for (j = 0; j < dset.chunk_counts; j++) {
			...
			case 0x5F5A4F4D:	//MOZ_:
				...
				break;
			default:
				fprintf(stderr, "unknown sub chunk %x\n", sub_chunk.magic);
				ret = -CUI_EMATCH;
				break;
			}	// sub chunck magic
			if (ret)
										...
										struct package_resource *pkg_res)
{
	 ...
	 * D:\Program\160-側傞偐側FD僋僀僘\050-傾乕僇僀僽嶌嬈\夋憸\Mem2.png
...
static int Xuse_BM_match(struct package *pkg)
{
	u16 bm;
	...
	if (pkg->pio->read(pkg, &bm, sizeof(bm))) {
		pkg->pio->close(pkg);
		return -CUI_EREAD;
	}
...
static int Xuse_BM_extract_resource(struct package *pkg,	
									struct package_resource *pkg_res)
{
...
static cui_ext_operation Xuse_BM_operation = {
	Xuse_BM_match,				/* match */
	NULL,						/* extract_directory */
	NULL,						/* parse_resource_info */
	Xuse_BM_extract_resource,	/* extract_resource */
	Xuse_arc_save_resource,		/* save_resource */
	Xuse_arc_release_resource,	/* release_resource */
	Xuse_arc_release			/* release */
};
...
int CALLBACK Xuse_register_cui(struct cui_register_callback *callback)
{
	...
	if (callback->add_extension(callback->cui, _T(".bin"), _T(".bmp"), 
		NULL, &Xuse_BM_operation, CUI_EXT_FLAG_PKG))
			return -1;
