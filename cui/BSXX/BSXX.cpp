...
/* 接口数据结构: 表示cui插件的一般信息 */
struct acui_information BSXX_cui_information = {
	_T("BISHOP"),			/* copyright */
	_T("BSXX システム"),	/* system */
	_T(".bsa"),				/* package */
	_T("0.5.1"),			/* revision */
...
/* 封包匹配回调函数 */
static int BSXX_bsa_match(struct package *pkg)
{
	s8 magic[8];
	u16 version;
...
/* 封包索引目录提取函数 */
static int BSXX_bsa_extract_directory(struct package *pkg,
...
/* 封包索引项解析函数 */
static int BSXX_bsa_parse_resource_info(struct package *pkg,
...
/* 封包资源提取函数 */
static int BSXX_bsa_extract_resource(struct package *pkg,
									struct package_resource *pkg_res)
{
	BYTE *compr, *uncompr;
	...
	if (!strncmp((char *)compr, "BSS-Graphics", 13)) {
		...
		if (bsg_header->mode == 1) {
			if (bsg_header->color_type == 2)
				...
				for (DWORD p = 0; p < bsg_header->uncomprlen; p += 4) {
					uncompr_data[3] = 0xff;
					uncompr_data += 4;
				}				
...
/* 资源保存函数 */
static int BSXX_bsa_save_resource(struct resource *res, 
								 struct package_resource *pkg_res)
{
...
/* 封包资源释放函数 */
static void BSXX_bsa_release_resource(struct package *pkg, 
									  struct package_resource *pkg_res)
{
...
/* 封包卸载函数 */
static void BSXX_bsa_release(struct package *pkg, 
							 struct package_directory *pkg_dir)
{
...
/* 封包处理回调函数集合 */
static cui_ext_operation BSXX_bsa_operation = {
	BSXX_bsa_match,					/* match */
	BSXX_bsa_extract_directory,		/* extract_directory */
	BSXX_bsa_parse_resource_info,	/* parse_resource_info */
	BSXX_bsa_extract_resource,		/* extract_resource */
	BSXX_bsa_save_resource,			/* save_resource */
	BSXX_bsa_release_resource,		/* release_resource */
	BSXX_bsa_release				/* release */
};

/********************* bsc *********************/

/* 封包匹配回调函数 */
static int BSXX_bsc_match(struct package *pkg)
{
	s8 magic[16];
...
/* 封包索引目录提取函数 */
static int BSXX_bsc_extract_directory(struct package *pkg,
									 struct package_directory *pkg_dir)
{
...
/* 封包索引项解析函数 */
static int BSXX_bsc_parse_resource_info(struct package *pkg,
									   struct package_resource *pkg_res)
{
...
/* 封包资源提取函数 */
static int BSXX_bsc_extract_resource(struct package *pkg,
									struct package_resource *pkg_res)
{
	BYTE *compr, *uncompr;
	DWORD uncomprlen, comprlen;
...
static void BSXX_bsc_release_resource(struct package *pkg, 
									 struct package_resource *pkg_res)
{
	if (pkg_res->actual_data) {
		free(pkg_res->actual_data);
...
/* 封包处理回调函数集合 */
static cui_ext_operation BSXX_bsc_operation = {
	BSXX_bsc_match,					/* match */
	BSXX_bsc_extract_directory,		/* extract_directory */
	BSXX_bsc_parse_resource_info,	/* parse_resource_info */
	BSXX_bsc_extract_resource,		/* extract_resource */
	BSXX_bsa_save_resource,			/* save_resource */
	BSXX_bsc_release_resource,		/* release_resource */
	BSXX_bsa_release				/* release */
};

/* 接口函数: 向cui_core注册支持的封包类型 */
int CALLBACK BSXX_register_cui(struct cui_register_callback *callback)
{
	/* 注册cui插件支持的扩展名、资源放入扩展名、处理回调函数和封包属性 */
	if (callback->add_extension(callback->cui, _T(".bsa"), NULL, 
		NULL, &BSXX_bsa_operation, CUI_EXT_FLAG_PKG | CUI_EXT_FLAG_DIR
		| CUI_EXT_FLAG_NOEXT))
			return -1;

	if (callback->add_extension(callback->cui, _T(".bsc"), NULL, 
		NULL, &BSXX_bsc_operation, CUI_EXT_FLAG_RES | CUI_EXT_FLAG_DIR))
			return -1;
